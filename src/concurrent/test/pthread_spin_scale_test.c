...
void pspinlock_lock_test(size_t loops, unsigned workers) {
    ...
    free(t_handles);
    pthread_spin_destroy(&test_data.pspinlock);
}

CTEST(pspinlock, lock) {
    pspinlock_lock_test(10000000, 1);
}

CTEST(pspinlock, lock4) {
    pspinlock_lock_test(10000000, 4);
}

CTEST(pspinlock, lock8) {
    pspinlock_lock_test(10000000, 8);
}

CTEST(pspinlock, lock16) {
    pspinlock_lock_test(10000000, 16);
}

CTEST(pspinlock, lock64) {
    pspinlock_lock_test(10000000, 64);
}
